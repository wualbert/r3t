...
class RGRRT(BasicRRT):
    def __init__(self, root_state, sampler, reached_goal_function, plan_collision_free_path_towards, state_tree=StateTree(), rewire_radius = None):
        ...
        reachable_states = self.get_reachable_states(root_state)
    ...
    def create_child_node(self, parent_node, child_state, cost_from_parent, path_from_parent):
        ...
        new_node = Node(child_state, parent=parent_node, path_from_parent=path_from_parent, cost_from_parent=cost_from_parent)
        parent_node.children.add(new_node)
        reachable_states = self.get_reachable_states(child_state)
        for rs in reachable_states:
            rs_hash = hash(str(rs))
            self.reachable_set_tree.insert(rs_hash, rs)
    ...
    def force_extend(self, new_state, nearest_node):
        # test for possibility to extend
        new_node = self.create_child_node(nearest_node, new_state, np.linalg.norm(new_state-nearest_node.state), new_state)
        return True, new_node
